...
tokenizer = LlamaTokenizer.from_pretrained(save_path, local_files_only=True, legacy=False)
...
class WrappedVisionTower(torch.nn.Module):
    ...
    def forward(self, images):
        ...
        return out.last_hidden_state[:, 0]  # [CLS] 토큰

model.get_model().vision_tower = WrappedVisionTower(vision_tower)
...
def inference(qs, image_file) :
  ...
  qs += '\n' + DEFAULT_IM_START_TOKEN + ' ' + ' '.join([DEFAULT_IMAGE_PATCH_TOKEN] * image_token_len) + ' ' + DEFAULT_IM_END_TOKEN
